...
export default function RouteSidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => { if (!open) return; const onKey = (event: KeyboardEvent) => { if (event.key === "Escape") onClose(); if (event.key === "Enter") { event.preventDefault(); document.querySelector(routes[selected].target)?.scrollIntoView({ behavior: "smooth", block: "start" }); onClose(); } }; document.body.style.overflow = "hidden"; window.addEventListener("keydown", onKey); return () => { document.body.style.overflow = ""; window.removeEventListener("keydown", onKey); }; }, [open, onClose, selected]);
  if (!open) return null;
  ...
  return <div className="route-sidebar-overlay" role="presentation"><button className="route-sidebar-backdrop" aria-label="Close route navigator" onClick={onClose} /><aside className="route-sidebar" aria-label="CookeByte route navigator">
    ...
    <div className="route-sidebar__wheel"><OptionWheel items={routes.map((route) => route.label)} defaultSelected={selected} onChange={(index) => setSelected(index)} textColor="#636363" activeColor="#f4f0e8" /></div>
